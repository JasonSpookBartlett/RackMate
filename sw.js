const CACHE='rackmate-v2.71-beta';
const STATIC_ASSETS=[
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    await cache.addAll(STATIC_ASSETS.map(url=>url+'?v=2.71'));
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    try{
      ...
      return (await caches.match(request)) || (await caches.match(url.pathname+'?v=2.71'));
    }
  })());
});
